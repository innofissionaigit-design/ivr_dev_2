...
# MERGE NOTE (sourav) -- follow-up to the C4/C5/C6 hybrid block further
# down in _continue_pending (search "was_correction = pending.pop"). That
# block only wires dev_sourav's correction-acknowledgment layer into ONE of
# dev_sourav's three correction entry points -- the mid-collection one,
# where `awaiting` names the field currently on the table. The other two
# are both entirely dev_sourav's own code, untouched by the original 8
# conflicts (so they were merged as-is, not part of that hybrid): the
# "confirm_booking" readback shortcut, and the "confirm_correction"
# field-naming state's own one-breath shortcut. Both used to write
# `pending["slots"][field] = new_value` directly for a corrected date,
# bypassing _apply_booking_date() entirely -- so a date corrected through
# either path kept its OLD date_said/date_weekday/date_checked/
# date_candidates/date_range sitting in slots under the NEW date, and
# never re-hit _open_booking() to re-check availability with clinic-api
# before the caller re-confirmed. This helper, plus the
# _BOOKING_REOPEN_FIELDS check added at each of those two call sites,
# closes that gap the same way C4/C5/C6 already closes it for the
# mid-collection path: route "date" through _apply_booking_date()
# (dev_rajarshee) so stale metadata never survives a correction, and
# re-enter _open_booking() for "date"/"time_slot" so a corrected value
# that makes the booking unavailable is caught before the caller
# re-confirms it (same E13-S6 reasoning _open_booking's own docstring
# gives), rather than confirmed on stale data and refused at the write.
# "time_slot" has no metadata of its own to clear (_apply_booking_date
# only exists for "date"), so it is written directly; every other
# correctable field here (phone, patient_name) is unaffected by
# availability and is also written directly.
def _apply_booking_correction(slots: dict, field: str, value: object, text: str) -> None:
    if field == "date":
        _apply_booking_date(slots, value, spoken_day_word(text) or "date")
    else:
        slots[field] = value


# Fields whose correction can change whether the booking is still
# available -- mirrors the tuple the C4/C5/C6 block already checks
# (`awaiting in ("date", "time_slot")`) rather than duplicating a second,
# possibly-drifting literal.
_BOOKING_REOPEN_FIELDS = ("date", "time_slot")
...
async def _continue_pending(session: CallSession, text: str) -> bool:
    ...
    if awaiting == "confirm_booking":
        ...
        if correction is not None:
            ...
            # MERGE NOTE (sourav) -- see _apply_booking_correction()'s own
            # note above (near _apply_booking_date) for why this no longer
            # writes pending["slots"] directly: a date corrected here now
            # clears its stale metadata and, together with "time_slot",
            # re-enters _open_booking() to re-check availability before
            # the booking is re-confirmed, instead of jumping straight
            # back to booking_confirmation_prompt() on data that may no
            # longer be bookable.
            _apply_booking_correction(pending["slots"], field, new_value, text)
            pending["retries"] = 0
            ack = correction_acknowledged_reply(field, pending["slots"], language=language)
            if field in _BOOKING_REOPEN_FIELDS and pending["slots"].get("doctor_name"):
                await _speak(session, ack)
                await _open_booking(session, pending["slots"], language,
                                    turns=pending.get("turns", 1),
    ...
    if awaiting == "confirm_correction":
        ...
        new_value = parser(text) if parser else None
        if new_value is not None:
            # MERGE NOTE (sourav) -- same follow-up as the "confirm_booking"
            # readback shortcut above: routes a date correction through
            # _apply_booking_correction() instead of writing
            # pending["slots"] directly, so stale date metadata is cleared
            # and a date/time_slot correction re-enters _open_booking() to
            # re-check availability before this state hands back to the
            # readback or the next missing field -- see
            # _apply_booking_correction()'s own note (near
            # _apply_booking_date) for the full reasoning.
            _apply_booking_correction(pending["slots"], field, new_value, text)
            pending["retries"] = 0
            ack = correction_acknowledged_reply(field, pending["slots"], language=language)
            if field in _BOOKING_REOPEN_FIELDS and pending["slots"].get("doctor_name"):
    ...
    # MERGE NOTE (sourav) -- C4/C5/C6 (rule 2, conflict), resolved as one
    # hand-combined block per the owner's explicit decision: dev_rajarshee's
    # helper-based structure (date-recheck via _apply_booking_date,
    # re-open-booking via _open_booking, and the centralized
    # _next_booking_step) is kept as the base; dev_sourav's correction-
    # acknowledgment layer (was_correction / _correcting_field, from the
    # "The agent accepts a correction and restates" story) is layered on
    # top as its own short spoken line, immediately before whichever of
    # dev_rajarshee's two paths runs next. This is two short _speak() calls
    # back to back (the acknowledgment, then _open_booking's or
    # _next_booking_step's own line) rather than one combined sentence --
    # every OTHER correction spot in this file combines them into a single
    # _speak call ("ack + ' ' + prompt"); flagged for a listening-test check
    # before this ships, since it is a real (if small) behaviour difference.
    ...
    pending["retries"] = 0
    if was_correction:
        await _speak(session, correction_acknowledged_reply(awaiting, pending["slots"], language=language))
...
async def _answer_part(session: CallSession, text: str, part: dict) -> str:
    ...
    try:
        if intent == "test_rate":
            ...
            #
            # MERGE NOTE (sourav) -- C7 (rule 2, conflict): dev_rajarshee's
            # helper-based path is used wholesale here. dev_sourav's version at
            # this spot (KCD-448) fixed the exact same Bengali-only-prompt bug by
            # switching to booking_confirmation_prompt(..., language=detect_language(text));
            # dev_rajarshee's _open_booking(session, merged, detect_language(text), ...)
            # already threads the same detect_language(text) fix through, so nothing
            # from dev_sourav's side is lost by taking dev_rajarshee's version outright.
...
_MULTI_INTENT_NEEDS_SEPARATE_FLOW = {
    ...
    # MERGE NOTE (sourav) -- C8 (rule 2, conflict): union of both sides.
    # dev_rajarshee's additions:
    "reschedule_appointment", "cancel_appointment",
    # dev_sourav's additions:
    # ADDED BY SOURAV -- "Caller asks whether their result is dangerous"
